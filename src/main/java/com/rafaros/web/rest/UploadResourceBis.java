...
public class UploadResourceBis {

    @Value("${file.base-path}")
    private String storagePath;

    @PostMapping(value = "/upload-posts-image", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<String> uploadPostsImage(@RequestParam("file") MultipartFile file) {
        try {
            String originalFilename = file.getOriginalFilename();
            Path imagePath = Paths.get(storagePath).resolve(originalFilename);
            Files.copy(file.getInputStream(), imagePath, StandardCopyOption.REPLACE_EXISTING);
            return ResponseEntity.ok("../../../content/posts/" + originalFilename);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error uploading image.  TRACE is : " + e);
        }
    }

    @PutMapping(value = "/update-posts-image/{imageName}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<String> updatePostsImage(@PathVariable String imageName, @RequestParam("file") MultipartFile file) {
        try {
    ...
    @DeleteMapping("/delete-posts-image/{imageName}")
    public ResponseEntity<String> deletePostsImage(@PathVariable String imageName) {
        try {
