...
public class CustomerRepositoryWithBagRelationshipsImpl implements CustomerRepositoryWithBagRelationships {
    ...
    public Optional<Customer> fetchBagRelationships(Optional<Customer> customer) {
        return customer.map(this::fetchPreferredLocales);
    }
    ...
    public List<Customer> fetchBagRelationships(List<Customer> customers) {
        return Optional.of(customers).map(this::fetchPreferredLocales).orElse(Collections.emptyList());
    }

    Customer fetchPreferredLocales(Customer result) {
        return entityManager
            .createQuery(
                "select customer from Customer customer left join fetch customer.preferredLocales where customer is :customer",
                Customer.class
            )
    ...
    List<Customer> fetchPreferredLocales(List<Customer> customers) {
        ...
        List<Customer> result = entityManager
            .createQuery(
                "select distinct customer from Customer customer left join fetch customer.preferredLocales where customer in :customers",
                Customer.class
            )
