...
import com.rafaros.config.EmbeddedKafka;
import com.rafaros.config.KafkaSseConsumer;
import com.rafaros.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class MadaskillKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/madaskill-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/madaskill-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/madaskill-kafka/unregister"));
                return;
            }
        }
