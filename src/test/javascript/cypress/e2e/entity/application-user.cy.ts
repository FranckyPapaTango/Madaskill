...
describe('ApplicationUser e2e test', () => {
  ...
  const applicationUserSample = { phoneNumber: 'Fuerte', paysCountry: 'Architecte support Licensed' };
  ...
  describe('new ApplicationUser page', () => {
    ...
    it('should create an instance of ApplicationUser', () => {
      cy.get(`[data-cy="phoneNumber"]`).type('Qatar').should('have.value', 'Qatar');

      cy.get(`[data-cy="email"]`).type('Dbora99@gmail.com').should('have.value', 'Dbora99@gmail.com');

      cy.get(`[data-cy="libelleAdresse"]`).type('generation b Seychelles').should('have.value', 'generation b Seychelles');

      cy.get(`[data-cy="villeTown"]`).type('Checking invoice').should('have.value', 'Checking invoice');

      cy.get(`[data-cy="paysCountry"]`).type('Buckinghamshire').should('have.value', 'Buckinghamshire');
