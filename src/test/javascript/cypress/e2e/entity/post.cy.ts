...
describe('Post e2e test', () => {
  ...
  const postSample = { title: 'Stagiaire Borders', datePersistence: '2023-10-24T03:18:16.578Z' };
  ...
  describe('new Post page', () => {
    ...
    it('should create an instance of Post', () => {
      ...
      cy.get(`[data-cy="body"]`).type('c Chicken').should('have.value', 'c Chicken');

      cy.get(`[data-cy="categorie"]`).type('PCI b Salad').should('have.value', 'PCI b Salad');

      cy.get(`[data-cy="resumee"]`).type('Intelligent').should('have.value', 'Intelligent');

      cy.get(`[data-cy="auteur"]`).type('matrix').should('have.value', 'matrix');

      cy.get(`[data-cy="datePersistence"]`).type('2023-10-23T21:08').blur().should('have.value', '2023-10-23T21:08');
